...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!installation) {
      throw new Error("Unable to create user credential for openvisio");
    }
  } catch (error: unknown) {
  ...
  return res
    .status(200)
    .json({ url: returnTo ?? getInstalledAppPath({ variant: "conferencing", slug: "openvisio" }) });
}
