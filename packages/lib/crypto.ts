...
/**
 * SEC-100: symmetric encryption for credentials at rest.
 *
 * Two formats are supported on the decrypt path:
 *
 *   v1 (legacy):  "<hex(iv)>:<hex(ciphertext)>"           AES-256-CBC, no MAC
 *   v2 (current): "v2:<hex(iv)>:<hex(tag)>:<hex(ciphertext)>"  AES-256-GCM
 *
 * `symmetricEncrypt` always produces v2. `symmetricDecrypt` detects the
 * format from the prefix — this gives us a lazy re-encrypt: every time
 * a credential round-trips through the codebase (e.g. OAuth refresh),
 * it ends up stored as v2 on the next write. No big-bang migration.
 *
 * Why GCM:
 *  - CBC has no integrity check; any random ciphertext "decrypts" to
 *    garbage with no error, which made the legacy `should fail with
 *    wrong key` test deliberately flaky.
